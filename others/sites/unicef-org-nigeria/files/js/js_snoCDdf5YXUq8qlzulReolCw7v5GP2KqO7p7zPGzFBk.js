...
(function ($, Drupal, drupalSettings) {
  ...
  Drupal.theme.noTranslationMsg = function (settings) {
    var html = '';
    html += '<div class="alert alert--language-message" role="alert">';
    html += '<div class="language-selection-wrapper col-md-12">';
    html += '<div class="language-selection">';
    html += '<div class="language-selection--text col-md-8">';
    html += '<p>' + settings.title + '</p>';
    html += '</div>';
    html += '<div class="language-selection--button col-md-3">';
    // html += '<a href="javascript:history.back()" class="btn cta--primary">Go Back</a>';
    html += '</div>';
    html += '<div class="language-selection--close-button col-md-1">';
    html += '<a href="#" role="button" class="close" data-dismiss="alert" aria-label="Close">';
    html += '<svg class="svg-icon__caption-close-language"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="' + drupalSettings.path.baseUrl + 'themes/custom/unicef/assets/dist/images/sprites/symbols/sprites.svg#icon__caption-close"></use></svg>';
    html += '</a>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
  ...
  Drupal.behaviors.fixNoTranslationLinks = {
    attach: function (context, settings) {
      $('a.language-link.has-no-translation', context).once("language-link-has-no-translation").on("click", function (e) {
        // Apply the myCustomBehaviour effect to the elements only once.
        var selLang = $(this).attr("hreflang"); 
        var langTitle = "";

        if (drupalSettings.msgTrans.currentPage[selLang]) {
          langTitle = drupalSettings.msgTrans.currentPage[selLang].title;
        }
        else {
          langTitle = drupalSettings.msgTrans.currentPage.title;
        }

        if ($(".alert.alert--language-message").length == 0) {
            var msgBlockElement = $(Drupal.theme('noTranslationMsg', { title: langTitle }));
            $('.region.region-header').append(msgBlockElement);
            // Add mobile class if required.
            jQuery(window).resize();
        }
        else {
	        if ($(".alert.alert--language-message .language-selection--text p").length == 0) {
                $(".alert.alert--language-message .item.item--message").html(langTitle);
            }
	        else { 
                $(".alert.alert--language-message .language-selection--text p").html(langTitle);
            }
        }
